export default async function handler(req, res) {
  ...
  const messageText = typeof bodyMsg === 'string' ? bodyMsg : '';
  ...
  const pinterestMatch = messageText.match(/(https:\/\/(www\.)?pinterest\.(com|in)\/pin\/\d+\/?|https:\/\/pin\.it\/[a-zA-Z0-9]+)/i);
  if (!pinterestMatch) {
  ...
  const PINTEREST_API_BASE = process.env.PINTEREST_API_BASE;
  const WHATSAPP_INSTANCE_ID = process.env.WHATSAPP_INSTANCE_ID;
  const WHATSAPP_ACCESS_TOKEN = process.env.WHATSAPP_ACCESS_TOKEN;

  if (!PINTEREST_API_BASE || !WHATSAPP_INSTANCE_ID || !WHATSAPP_ACCESS_TOKEN) {
